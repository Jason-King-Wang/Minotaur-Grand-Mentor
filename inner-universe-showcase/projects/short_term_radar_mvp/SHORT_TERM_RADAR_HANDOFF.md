...
- Archived the 2026-05-12 full remaining tasks attachment under `docs/`.
- Added processed-table CSV fallback so storage/upsert/coverage tests work even when pandas/parquet is unavailable.
- Added official open-data dry-run/collector skeleton for institutional trading, margin/short, material events, corporate actions, valuation, and symbol master. Live fetch failures degrade cleanly and empty fetches do not write processed tables.
...
- `py -3.11 -m pytest -q tests\short_term_radar` -> `50 passed`
- `py -3.11 compileall` for `short_term_radar` and `tests/short_term_radar` -> passed
...
- Institutional trading, margin/short, material events, corporate actions, valuation, and symbol master now have official dry-run endpoint skeletons. The remaining work is live endpoint verification and full field mapping against real official payloads.
