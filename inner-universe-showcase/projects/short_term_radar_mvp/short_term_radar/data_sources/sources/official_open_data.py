...
ENDPOINTS: dict[str, dict[str, list[tuple[str, str]]]] = {
    "institutional_trading": {
        "TWSE": [("twse_t86", "https://openapi.twse.com.tw/v1/exchangeReport/T86")],
        "TPEX": [("tpex_3insti", "https://www.tpex.org.tw/openapi/v1/tpex_mainboard_3insti")],
    },
    "margin_short": {
        "TWSE": [
    ...
    def build_requests(self, dataset: str, market: str = "all", date: str | None = None) -> list[OfficialRequest]:
        ...
        for item_market in markets:
            for endpoint_name, url in ENDPOINTS[dataset].get(item_market, []):
                requests.append(OfficialRequest(dataset, item_market, endpoint_name, url, date))
        return requests
