...
def summarize_backtest(
    ...
    drawdowns = [
        float(row["forward_max_drawdown"])
        for row in candidate_rows
        if row.get("forward_max_drawdown") is not None
    ]
    ...
    return {
        ...
        "avg_forward_return": _avg(max_returns),
        "median_forward_return": median(max_returns) if max_returns else None,
        "max_forward_return": max(max_returns) if max_returns else None,
